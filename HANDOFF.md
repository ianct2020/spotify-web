...
- Git: rama `main` (v=53 — se sacó Álbum similar; likes/ediciones en escuchados)
- Cache bust: **`?v=53`**
- **IMPORTANTE — cache de módulos:** el build (`build.sh`) ahora agrega `?v=N` a TODOS los imports relativos de `docs/js`. Bumpear la versión en index.html invalida todo el árbol de módulos (antes solo se invalidaba app.js y los módulos importados quedaban cacheados viejos → causó bugs fantasma). No hace falta versionar imports a mano.
- **Álbum similar (#discover) fue ELIMINADO en v=53.** La búsqueda de álbumes de artistas similares nunca rindió confiable en el entorno de Ian (siempre "0 lanzamientos vistos"); él pidió sacarla. `features/discover-album.js` borrado.
...
- `v=53` (f2d258d): **Se sacó Álbum similar + ediciones deluxe.** Eliminada la feature `#discover` (link, ruta, import, card, y `features/discover-album.js`). En Álbumes escuchados, botón "💿 Ediciones deluxe (N)" que lista los álbumes registrados que son ediciones (detectado con `isEdition` = `norm(name) !== norm(baseName(name))`), con link a Spotify para reemplazar a mano (opción a que pidió Ian).
- `v=50-52`: (50) Álbum similar pasó a búsqueda de álbumes por **texto libre** (el filtro `artist:"..."&type=album` devuelve 0 post-migración) + sort deselect en escuchados. (51) botón "🎧 Quizás sin registrar" (álbumes con N+ likes fuera del registro) + la "S" del sidebar lleva a #home. (52) **build.sh versiona todos los imports** (fix del cache de módulos viejos) + umbral ajustable 3/4/5/6/8 en "quizás sin registrar". **Pendiente futuro:** Ian pidió a Spotify su historial de reproducción extendido (JSON) → cuando llegue, armar importador que agrupe por álbum (mejor fuente para "qué escuchó" que la heurística de likes).
