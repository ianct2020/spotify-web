...
import { loadHistoryStats, isOwner, ownerLockedMessage } from './history-data.js?v=224';
import { escapeHtml, pageHeader } from '../ui/components.js?v=224';
import { getPreview } from '../api/preview-providers.js?v=224';
import { getArtistLikePreview, getAlbumLikePreview } from '../util/artist-preview.js?v=224';
import { attachHover } from '../ui/preview-player.js?v=224';
...
async function montarAperturaSiSePuede(ruta) {
  ...
  if (!host) return;
  try {
    const mod = await import('../features/wrapped-apertura.js?v=224');
