...
import { loadHistoryStats, isOwner, ownerLockedMessage } from './history-data.js?v=229';
import { escapeHtml, pageHeader } from '../ui/components.js?v=229';
import { getPreview } from '../api/preview-providers.js?v=229';
import { getArtistLikePreview, getAlbumLikePreview } from '../util/artist-preview.js?v=229';
import { attachHover } from '../ui/preview-player.js?v=229';
...
async function montarAperturaSiSePuede(ruta) {
  ...
  if (!host) return;
  try {
    const mod = await import('../features/wrapped-apertura.js?v=229');
